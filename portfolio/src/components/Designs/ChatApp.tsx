      ...
      <div className="designs-card">
        ...
        <p><h2>IV. Design</h2></p>
            ...
            <p><h3>Load Balancers</h3></p>
                <p>
                    The load balancing layer acts as a reverse proxy for the chat service. To prevent the chat service from becoming a point of failure at scale, I designed for it to be horizontally scalable. Furthermore, I designed the load balancing layer to implement a <a href="https://en.wikipedia.org/wiki/Consistent_hashing#Basic_Technique" rel="noreferrer noopener" target="blank">consistent hashing strategy</a> with users with the same session id being routed to the same server. Since this helps us avoid the problem of users being reassigned to new servers as servers are both added and removed from the cluster, we can also add an in-memory cache, such as Memcached, that'll lower the need to go grab message history from the database.
                </p>
                <br />
            <p><h3>More about Chat Service</h3></p>
            <p>
                After a user initiates a new chat via their browser, chat service opens a bi-directional communication channel for the BFF to communicate with the service. I chose this method instead of polling, because the latter would put considerable load on our servers at scale. Additionally, by keeping an open connection, we will likely experience lower latencies.
            </p>
            <p>
        ...
        <p><h3>Database</h3></p>
            ...
            <p>
                One problem I wanted to be mindful of should we go with a self-managed database solution (to save money) is how we'll handle both scaling reads and making our database solution fault-tolerant and highly available. In this regard, having a leader-replica configuration should work. For a single region deployment, one leader with two replicas in different availability zones is more than sufficient. We could even add in <a href="https://proxysql.com/">ProxySQL</a> to handle delegating all writes to the leader and reads to the replicas, but adding that in the service configs/logic would be a smaller lift.
            </p>
            <p>
